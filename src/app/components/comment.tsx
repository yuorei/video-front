...
const Comments: React.FC<CommentsProps> = ({ videoID }) => {
    const { loading, error, data, refetch } = useQuery<CommentsByVideoData>(COMMENTS_BY_VIDEO, {
        variables: { videoID },
    });
    ...
    const [showComments, setShowComments] = useState<boolean>(window.innerWidth > 1280);
    ...
    return (
        <div className="mt-4">
            <h2 className="text-lg font-semibold mb-2">{data?.commentsByVideo.length}件のコメント</h2>
            <button onClick={toggleComments} className="bg-black border border-white text-white font-bold py-2 px-4 rounded">
